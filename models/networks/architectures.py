...
class ResNetDecoder(nn.Module):
    ...
    def __init__(self, opt, channels_in=64, channels_out=3, norm="batch"):
        ...
        use_tanh = opt.use_tanh
        self.use_tanh = use_tanh
        if use_tanh:
            self.norm = nn.Tanh()

    def forward(self, x, scale_factor):
        if self.opt.predict_residual:
            if self.use_tanh:
                return self.norm(self.eblocks(x * scale_factor) + x[:, 0:3])
            else:
                return self.eblocks(x * scale_factor) + x[:, 0:3]
        else:
            if self.use_tanh:
                return self.norm(self.eblocks(x * scale_factor))
            else:
                return self.eblocks(x * scale_factor)
    # def forward(self, x):
